...
#define ELRS_DIAG_RX_LUA_UL SIW917_ELRS_LUA_PROGRESS_DIAG
...
#define ELRS_DIAG_LUA_DISCOVERY SIW917_ELRS_LUA_PROGRESS_DIAG
      ...
      if (validateCrsfFrame(frame, &frameLen)) {
        ...
        if (crsfSerialShouldForwardFrame(frame, frameLen)) {
          crsfRouter.processMessage(
              nullptr, reinterpret_cast<const crsf_header_t *>(frame));
        }
      }
      pos = 0;
      expectedLen = 0;
...
static inline bool isGfskRate(const expresslrs_mod_settings_s *params);
...
static void ICACHE_RAM_ATTR updatePhaseLock() {
  if (connectionState != disconnected && PFDloop.hasResult()) {
    ...
    int32_t normalizedOffset = rawOffset;
#if SIW917_ELRS_LR2021_PERIOD_NORMALIZE_PFD
    if (ExpressLRS_currAirRate_Modparams != nullptr &&
        ExpressLRS_currAirRate_Modparams->interval != 0U) {
      const int32_t interval =
          (int32_t)ExpressLRS_currAirRate_Modparams->interval;
      const int32_t halfInterval = interval / 2;
      normalizedOffset = rawOffset % interval;
      if (normalizedOffset > halfInterval) {
        normalizedOffset -= interval;
      } else if (normalizedOffset < -halfInterval) {
        normalizedOffset += interval;
      }
    }
#endif
    ...
    int32_t phaseShift =
        connectionState != connected ? (normalizedOffset >> 1) : (offset >> 2);
...
static void ResetDownlinkTelemetryStateAfterLoss() {
  ...
  deferredTelemetrySendGemini = false;
  deferredTelemetryNonce = 0;
  deferredTelemetryFhss = 0;
  telemetryExitCritical(primask);
#endif
#if SIW917_ELRS_LR2021_TXDONE_WATCHDOG
  lr2021TelemetryTxPending = 0;
  lr2021TelemetryTxStartUs = 0;
#endif
  InvalidatePrebuiltTelemetry();
}

static void RecoverRadioAfterGfskLoss() {
  if (!isGfskRate(ExpressLRS_currAirRate_Modparams)) {
    return;
  }

  Radio.SetTxIdleMode();
  Radio.ClearIrqStatus(SX12XX_Radio_All);
  Radio.ClearRxFifo(SX12XX_Radio_All);
}
...
static void LostConnection(bool resumeRx) {
  ...
  ResetDownlinkTelemetryStateAfterLoss();

  hwTimer::resetFreqOffset();
  RecoverRadioAfterGfskLoss();
...
ClampTelemetryDenomForSiw917(uint8_t denom, uint8_t rateIndex) {
  const expresslrs_mod_settings_s *const params =
      get_elrs_airRateConfig(rateIndex);
  if (denom != 1 &&
      (params->radio_type == RADIO_TYPE_LR1121_GFSK_900 ||
       params->radio_type == RADIO_TYPE_LR1121_GFSK_2G4) &&
      denom < SIW917_ELRS_LR2021_GFSK_MIN_TLM_DENOM) {
    return SIW917_ELRS_LR2021_GFSK_MIN_TLM_DENOM;
  }

  if (denom != 1 && denom < SIW917_ELRS_LR2021_MIN_TLM_DENOM) {
    return SIW917_ELRS_LR2021_MIN_TLM_DENOM;
  }
...
ProcessRfPacket_SYNC(uint32_t const now, OTA_Sync_s const *const otaSync) {
  ...
  uint8_t proposedRateIndex = 0;
  const expresslrs_RFrates_e proposedRateEnum =
      (expresslrs_RFrates_e)otaSync->rfRateEnum;
  if (!enumRatetoIndex(proposedRateEnum, proposedRateIndex) ||
      !isSupportedRFRate(proposedRateIndex)) {
    DBGLN("Unsupported rate %u", (unsigned)proposedRateEnum);
    return false;
  }
  ...
  ExpressLRS_nextAirRateIndex = proposedRateIndex;
  ...
  if (ExpressLRS_currTlmDenom != TlmDenom) {
    ...
    if (requestedTlmDenom != TlmDenom) {
      DBGLN("TLM denom clamped 1:%u -> 1:%u for SiW917 LR2021",
  ...
  const bool syncNeedsStateUpdate =
      connectionState == disconnected || OtaNonce != otaSync->nonce ||
      FHSSgetCurrIndex() != otaSync->fhssIndex ||
      connectionHasModelMatch != modelMatched;

  if (syncNeedsStateUpdate) {
#if defined(SIW917_ELRS_LR2021_GFSK_SOFT_SYNC_RESYNC) &&                    \
    SIW917_ELRS_LR2021_GFSK_SOFT_SYNC_RESYNC
    const bool gfskRate =
        ...
        (ExpressLRS_currAirRate_Modparams->radio_type ==
         ...
         ExpressLRS_currAirRate_Modparams->radio_type ==
             RADIO_TYPE_LR1121_GFSK_900);
    const bool sameCurrentRate =
        ExpressLRS_currAirRate_Modparams != nullptr &&
        ExpressLRS_nextAirRateIndex == ExpressLRS_currAirRate_Modparams->index;
    if (gfskRate && sameCurrentRate && connectionState == connected &&
        connectionHasModelMatch == modelMatched) {
      const uint8_t previousNonce = OtaNonce;
      const uint8_t previousFhss = FHSSgetCurrIndex();
      FHSSsetCurrIndex(otaSync->fhssIndex);
      OtaNonce = otaSync->nonce;
      connectionHasModelMatch = modelMatched;
      static uint8_t gfskSoftSyncLogCount = 0;
      if (gfskSoftSyncLogCount < 8U) {
        DBGLN("GFSK_SYNC_SOFT conn:%d rxst:%d nonce:%u->%u fhss:%u->%u",
              connectionState, RXtimerState, previousNonce, otaSync->nonce,
              previousFhss, otaSync->fhssIndex);
        gfskSoftSyncLogCount++;
      }
      return false;
    }
#endif
...
static volatile uint8_t scanCrcRejectLen = 0;
static volatile uint8_t scanCrcRejectBytes[13] = {};
#endif
...
#if defined(SIW917_ELRS_LR2021_LINK_PROGRESS_STATUS_SPI) &&                   \
    SIW917_ELRS_LR2021_LINK_PROGRESS_STATUS_SPI
  const uint8_t radioStatusOk =
      lr1121_get_status(&radioStat1, &radioStat2, &radioIrqByte) ? 1 : 0;
#else
  const uint8_t radioStatusOk = 0;
#endif
...
static void printLossPacketStats() {
  ...
  DBGLN("LOSSSTAT irq:%lu/%lu/%lu/%lu/0x%08lX dio:%d "
        ...
        "crcN:%d/%u/%u/%u/%lu/%lu "
        "freq:%lu/%lu lq:%u/%u tlm:%lu/%u/%u ack:%lu/%u ulack:%lu/%u "
        "def:%lu/%lu/%lu rssi:%d snr:%d "
        "lat:%lu/%lu/%lu/%lu/%lu",
        ...
        telemetryBurstMax, (unsigned long)telemetryRcConfirmCount,
        telemetryLastRcConfirm ? 1 : 0,
        (unsigned long)telemetryDataUlAckCount,
        telemetryLastDataUlAck ? 1 : 0, (unsigned long)deferredQueueCount,
...
static int8_t ICACHE_RAM_ATTR __attribute__((unused))
countFhssHopDeltaForNonceCorrection(uint8_t const oldNonce,
                                    int8_t const nonceDelta) {
  if (ExpressLRS_currAirRate_Modparams == nullptr) {
    return 0;
  }

  const uint8_t hopInterval =
      ExpressLRS_currAirRate_Modparams->FHSShopInterval;
  if (hopInterval == 0U) {
    return 0;
  }
  ...
  int8_t hopDelta = 0;
  if (nonceDelta > 0) {
    for (int8_t step = 1; step <= nonceDelta; step++) {
      const uint8_t crossedNonce = (uint8_t)(oldNonce + step);
      if ((crossedNonce % hopInterval) == 0U) {
        hopDelta++;
      }
    }
  } else if (nonceDelta < 0) {
    for (int8_t step = 0; step > nonceDelta; step--) {
      const uint8_t crossedNonce = (uint8_t)(oldNonce + step);
      if ((crossedNonce % hopInterval) == 0U) {
        hopDelta--;
      }
...
static int8_t ICACHE_RAM_ATTR __attribute__((unused))
applyFhssCorrectionForNonceResync(uint8_t const oldNonce,
                                  int8_t const nonceDelta,
                                  uint8_t *const oldFhss,
                                  uint8_t *const newFhss) {
  const uint8_t before = FHSSgetCurrIndex();
  if (oldFhss != nullptr) {
    *oldFhss = before;
  }
  if (newFhss != nullptr) {
...
#if SIW917_ELRS_LR2021_GFSK_RESYNC_ADJUST_FHSS
  const int8_t hopDelta =
      countFhssHopDeltaForNonceCorrection(oldNonce, nonceDelta);
  const uint16_t sequenceCount = FHSSgetSequenceCount();
  if (hopDelta == 0 || sequenceCount == 0U) {
    return hopDelta;
  }

  int16_t correctedIndex = (int16_t)before + hopDelta;
  while (correctedIndex < 0) {
    correctedIndex += (int16_t)sequenceCount;
  }
  correctedIndex %= (int16_t)sequenceCount;
  FHSSsetCurrIndex((uint8_t)correctedIndex);

  if (newFhss != nullptr) {
    *newFhss = FHSSgetCurrIndex();
  }
  return hopDelta;
#else
  (void)oldNonce;
...
ProcessRFPacket(SX12xxDriverCommon::rx_status const status) {
  ...
  const uint32_t crcDiagDio1EdgeUs = lr1121_hal_get_last_dio1_edge_us();
  if (crcDiagDio1EdgeUs != 0U && ExpressLRS_currAirRate_Modparams != nullptr &&
      ExpressLRS_currAirRate_Modparams->interval != 0U) {
    const uint32_t edgeAgeUs = beginProcessing - crcDiagDio1EdgeUs;
    if (edgeAgeUs < 20000U) {
      const uint32_t interval = ExpressLRS_currAirRate_Modparams->interval;
      const uint32_t slots = (edgeAgeUs + (interval / 2U)) / interval;
      crcDiagEdgeAgeUs = edgeAgeUs;
      crcDiagEdgeSlots = slots > 255U ? 255U : (uint8_t)slots;
      crcDiagPacketNonce = (uint8_t)(OtaNonce - crcDiagEdgeSlots);
    }
  }
#endif
#endif
  ...
  if (connectionState == disconnected) {
    const uint8_t payloadLength =
        ExpressLRS_currAirRate_Modparams != nullptr
            ...
            : OTA4_PACKET_SIZE;
    scanRawLen =
        payloadLength < sizeof(scanRawBytes) ? payloadLength
                                             : (uint8_t)sizeof(scanRawBytes);
    for (uint8_t i = 0; i < scanRawLen; i++) {
      scanRawBytes[i] = Radio.RXdataBuffer[i];
    }
  }
#endif
  ...
  if (!crcValid) {
    ...
    if (gfskNonceResyncAllowed && lastCrcNonceMatched != 0xFF &&
      ...
      if (OtaValidatePacketCrcForNonce(&recoveredPacket,
                                       lastCrcNonceMatched)) {
        const uint8_t oldNonce = OtaNonce;
        uint8_t oldFhss = FHSSgetCurrIndex();
        uint8_t newFhss = oldFhss;
        const int8_t hopDelta = applyFhssCorrectionForNonceResync(
            oldNonce, nonceDelta, &oldFhss, &newFhss);
        OtaNonce = lastCrcNonceMatched;
        memcpy(otaPktPtr, &recoveredPacket, sizeof(*otaPktPtr));
        crcValid = true;
        gfskRecoveredNonce = true;
        if (gfskNonceResyncLogCount < 8U) {
          DBGLN("GFSK_NONCE_RESYNC conn:%d rxst:%d delta:%d old:%u new:%u fhss:%u->%u hop:%d max:%u type:%u",
                connectionState, RXtimerState,
                (int)nonceDelta, (unsigned)oldNonce,
                (unsigned)OtaNonce, (unsigned)oldFhss, (unsigned)newFhss,
                (int)hopDelta, (unsigned)maxNonceResyncDelta,
                (unsigned)type);
          gfskNonceResyncLogCount++;
        }
      }
    }
#endif
    ...
    if (connectionState == disconnected) {
      scanCrcRejectRate =
          ExpressLRS_currAirRate_Modparams != nullptr
              ? ExpressLRS_currAirRate_Modparams->index
              : 0xFF;
      scanCrcRejectType = type;
      scanCrcRejectLen = scanRawLen;
      for (uint8_t i = 0; i < sizeof(scanCrcRejectBytes); i++) {
        scanCrcRejectBytes[i] =
            i < scanRawLen ? scanRawBytes[i] : 0;
      }
      scanCrcRejectCount++;
    }
#endif
#if ELRS_DIAG_CRC_NONCE_WINDOW > 0
    if (gfskRate && connectionState != disconnected &&
        gfskCrcFailDiagCount < 16U) {
      const int8_t packetNonceDelta =
          lastCrcNonceMatched == 0xFF
              ? 127
              : (int8_t)(lastCrcNonceMatched - crcDiagPacketNonce);
      DBGLN("GFSK_CRC_FAIL conn:%d rxst:%d type:%u nonce:%u fhss:%u "
            "crcN:%d/%u/%u/%u/%lu/%lu edge:%lu/%u/%u/%d "
            "d:%02X %02X %02X %02X %02X %02X %02X %02X",
            connectionState, RXtimerState, (unsigned)type, OtaNonce,
            FHSSgetCurrIndex(), (int)lastCrcNonceDelta, lastCrcNonceExpected,
            lastCrcNonceMatched, lastCrcNonceType,
            (unsigned long)crcNonceDiagHitCount,
            (unsigned long)crcNonceDiagMissCount,
            (unsigned long)crcDiagEdgeAgeUs, (unsigned)crcDiagEdgeSlots,
            (unsigned)crcDiagPacketNonce, (int)packetNonceDelta,
            (unsigned)((const uint8_t *)&rawOtaPacket)[0],
            (unsigned)((const uint8_t *)&rawOtaPacket)[1],
            (unsigned)((const uint8_t *)&rawOtaPacket)[2],
            (unsigned)((const uint8_t *)&rawOtaPacket)[3],
            (unsigned)((const uint8_t *)&rawOtaPacket)[4],
            (unsigned)((const uint8_t *)&rawOtaPacket)[5],
            (unsigned)((const uint8_t *)&rawOtaPacket)[6],
            (unsigned)((const uint8_t *)&rawOtaPacket)[7]);
      gfskCrcFailDiagCount++;
    }
#endif
    return false;
  }
  ...
  if (ExpressLRS_currAirRate_Modparams != nullptr &&
      ExpressLRS_currAirRate_RFperfParams != nullptr &&
#if SIW917_ELRS_LR2021_GFSK_RESYNC_SKIP_PFD
      !gfskRecoveredNonce
#else
      true
#endif
...
#if SIW917_ELRS_LR2021_GFSK_RESYNC_SKIP_PFD
  } else if (gfskRecoveredNonce) {
    lastPfdSlackUs = 0;
    pfdSkippedNoTimestampCount++;
    PFDloop.reset();
#endif
  }
...
#if SIW917_ELRS_LR2021_GFSK_SKIP_IMMEDIATE_TOCK
      if (ExpressLRS_currAirRate_Modparams != nullptr &&
          (ExpressLRS_currAirRate_Modparams->radio_type ==
               RADIO_TYPE_LR1121_GFSK_900 ||
           ExpressLRS_currAirRate_Modparams->radio_type ==
               RADIO_TYPE_LR1121_GFSK_2G4)) {
        deliverImmediateTock = false;
      }
#endif
      hwTimer::resume(deliverImmediateTock);
    }
    return true;
  }
...
static void SIW917_ELRS_RAMFUNC_ATTR ICACHE_RAM_ATTR
TXdoneISRCommon(bool forceManualRx) {
#if SIW917_ELRS_LR2021_TXDONE_WATCHDOG
  lr2021TelemetryTxPending = 0;
#endif
#if SIW917_ELRS_TLM_TURNAROUND_TRACE
...
static void SIW917_ELRS_RAMFUNC_ATTR ICACHE_RAM_ATTR TXdoneISR() {
  TXdoneISRCommon(false);
}

static void SIW917_ELRS_RAMFUNC_ATTR ICACHE_RAM_ATTR TXdoneISRForceManualRx() {
  TXdoneISRCommon(true);
}
...
static void printRadioRxStats(const char *tag,
  ...
  if (connectionState != disconnected || params == nullptr) {
    return;
  }

  const bool expectGfsk = isGfskRate(params);
  ...
  uint16_t gfskLenError = 0;
  uint16_t gfskPreamble = 0;
  uint16_t gfskSyncOk = 0;
  uint16_t gfskSyncFail = 0;
  uint16_t gfskTimeout = 0;

  if (expectGfsk) {
    Radio.GetGfskRxStats(SX12XX_Radio_1, &gfskRxTotal, &gfskCrcError,
                         &gfskLenError, &gfskPreamble, &gfskSyncOk,
  ...
  static uint32_t lastTxIrqCount = 0;
  static uint32_t lastOtherIrqCount = 0;
  static uint32_t lastCrcPassCount = 0;
  static uint32_t lastCrcFailCount = 0;
  static uint32_t lastScanCrcRejectCount = 0;
  ...
  const uint32_t rejectCount = scanCrcRejectCount;
  const uint8_t rejectRate = scanCrcRejectRate;
  const uint8_t rejectType = scanCrcRejectType;
  const uint8_t rejectLen = scanCrcRejectLen;
  uint8_t rejectBytes[13] = {};
  for (uint8_t i = 0; i < sizeof(rejectBytes); i++) {
    rejectBytes[i] = scanCrcRejectBytes[i];
  }

  DBGLN("SCAN_PKT rate:%u irq:%lu/+%lu rx:%lu/+%lu tx:%lu/+%lu "
        "oth:%lu/+%lu last:0x%08lX ok:%lu/+%lu fail:%lu/+%lu "
        "rej:%lu/+%lu rr:%u rt:%u rl:%u "
        "d:%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
        (unsigned)params->index, (unsigned long)isrCount,
        (unsigned long)(isrCount - lastIsrCount), (unsigned long)rxIrqCount,
        (unsigned long)(rxIrqCount - lastRxIrqCount),
        (unsigned long)txIrqCount,
        (unsigned long)(txIrqCount - lastTxIrqCount),
        (unsigned long)otherIrqCount,
        (unsigned long)(otherIrqCount - lastOtherIrqCount),
        (unsigned long)lastIrq, (unsigned long)crcPassCount,
        (unsigned long)(crcPassCount - lastCrcPassCount),
        (unsigned long)crcFailCount,
        (unsigned long)(crcFailCount - lastCrcFailCount),
        (unsigned long)rejectCount,
        (unsigned long)(rejectCount - lastScanCrcRejectCount),
        (unsigned)rejectRate, (unsigned)rejectType, (unsigned)rejectLen,
  ...
  lastTxIrqCount = txIrqCount;
  lastOtherIrqCount = otherIrqCount;
  lastCrcPassCount = crcPassCount;
  lastCrcFailCount = crcFailCount;
  lastScanCrcRejectCount = rejectCount;
#endif
}
#endif
...
#if ELRS_DIAG_SCAN_TRACE || ELRS_DIAG_RF_RATE_LOG
  const uint8_t defaultTlmDenom = TLMratioEnumToValue(ModParams->TLMinterval);
#endif

#if ELRS_DIAG_SCAN_TRACE
  uint8_t effectivePreambleLen = ModParams->PreambleLen;
#if defined(SIW917_ELRS_LR2021_LORA_SF5_MIN_PREAMBLE) &&                     \
    SIW917_ELRS_LR2021_LORA_SF5_MIN_PREAMBLE > 0
  if (!isGfskRate(ModParams) &&
      (ModParams->sf == LR11XX_RADIO_LORA_SF5 ||
       ModParams->sf == LR11XX_RADIO_LORA_SF6) &&
      effectivePreambleLen < SIW917_ELRS_LR2021_LORA_SF5_MIN_PREAMBLE) {
    effectivePreambleLen = SIW917_ELRS_LR2021_LORA_SF5_MIN_PREAMBLE;
  }
#endif
  DBGLN("SCAN_SET idx:%u enum:%u kind:%c sf:%u bw:%u cr:%u pre:%u pay:%u "
        "int:%lu hop:%u freq:%lu cycle:%lu tlm:%u compat:%u bind:%u",
        (unsigned)index, (unsigned)ModParams->enum_rate,
        isGfskRate(ModParams) ? 'F' : 'L', (unsigned)ModParams->sf,
        (unsigned)ModParams->bw, (unsigned)ModParams->cr,
        (unsigned)effectivePreambleLen, (unsigned)ModParams->PayloadLength,
        (unsigned long)interval, (unsigned)ModParams->FHSShopInterval,
        (unsigned long)initFreq, (unsigned long)cycleInterval,
        (unsigned)defaultTlmDenom,
        (!isGfskRate(ModParams) && FHSSusePrimaryFreqBand &&
         ModParams->sf <= LR11XX_RADIO_LORA_SF6)
            ? 1
            : 0,
        bindMode ? 1 : 0);
...
#if defined(SIW917_ELRS_LR2021_FORCE_RF_TLM) && SIW917_ELRS_LR2021_FORCE_RF_TLM
  return false;
#else
...
#if SIW917_ELRS_LR2021_TXDONE_WATCHDOG
static uint32_t ICACHE_RAM_ATTR Lr2021LoraBandwidthHz(uint8_t bw) {
  switch (bw) {
...
static uint32_t ICACHE_RAM_ATTR Lr2021EstimateTxAirtimeUs() {
  const expresslrs_mod_settings_s *const params =
      ExpressLRS_currAirRate_Modparams;
  if (params == nullptr) {
    return 0;
  ...
  if (params->radio_type == RADIO_TYPE_LR1121_GFSK_900 ||
      params->radio_type == RADIO_TYPE_LR1121_GFSK_2G4) {
    const uint32_t payloadLength =
        (params->radio_type == RADIO_TYPE_LR1121_GFSK_2G4) ? 14U
                                                            : params->PayloadLength;
    const uint32_t bits = ((uint32_t)params->PreambleLen + 16U +
                           (payloadLength * 8U));
    return (uint32_t)(((uint64_t)bits * 1000000ULL + 299999ULL) /
                      300000ULL);
  }

  const uint32_t bwHz = Lr2021LoraBandwidthHz(params->bw);
  const uint32_t sf = params->sf;
  if (bwHz == 0 || sf < 5 || sf > 12) {
    return 0;
  }

  const uint32_t de = (sf >= 11 && bwHz <= 125000U) ? 1U : 0U;
  const uint32_t denom = 4U * (sf - (2U * de));
  const int32_t numerator =
      (int32_t)(8U * params->PayloadLength) - (int32_t)(4U * sf) + 28;
  uint32_t payloadSymbols = 8U;
  if (numerator > 0) {
    const uint32_t crSymbols =
        (params->cr <= LR11XX_RADIO_LORA_CR_4_8) ? params->cr
                                                 : (params->cr - 4U);
    payloadSymbols +=
        (((uint32_t)numerator + denom - 1U) / denom) * (crSymbols + 4U);
  }

  const uint32_t totalSymbolsX100 =
      ((uint32_t)params->PreambleLen + payloadSymbols) * 100U + 425U;
  return (uint32_t)(((uint64_t)totalSymbolsX100 * (1ULL << sf) *
                         1000000ULL +
                     (uint64_t)bwHz * 100ULL - 1ULL) /
                    ((uint64_t)bwHz * 100ULL));
}
...
#if SIW917_ELRS_LR2021_TXDONE_WATCHDOG_PEEK_IRQ
  const uint32_t peekIrqStatus = Radio.PeekIrqStatus(SX12XX_Radio_1);
  if (peekIrqStatus & LR1121_IRQ_TX_DONE) {
...
#if defined(SIW917_ELRS_LR2021_TLM_AFTER_TIMER_LOCK) &&                       \
    SIW917_ELRS_LR2021_TLM_AFTER_TIMER_LOCK
  const bool timerReady =
      (RXtimerState == tim_locked) ||
#if defined(SIW917_ELRS_LR2021_GFSK_TLM_BEFORE_TIMER_LOCK) &&                 \
    SIW917_ELRS_LR2021_GFSK_TLM_BEFORE_TIMER_LOCK
      ((connectionState == connected) &&
       (ExpressLRS_currAirRate_Modparams != nullptr) &&
       isGfskRate(ExpressLRS_currAirRate_Modparams));
#else
      false;
#endif
  if (connectionState != connected || !timerReady) {
    alreadyTLMresp = true;
    telemetrySuppressedCount++;
#if SIW917_ELRS_HOTPATH_TIMING_DIAG
...
#endif
#if ELRS_DIAG_SCAN_TRACE
    DBGLN("SCAN_CYCLE next:%u prevInterval:%lu dwell:%lu mult:%u",
          (unsigned)currentScanIndex, (unsigned long)cycleInterval,
          (unsigned long)dwellMs, (unsigned)RFmodeCycleMultiplier);
#endif
...
void elrs_loop(void) {
...
#if defined(SIW917_ELRS_LR2021_POLL_IRQ_WHEN_NO_DIO) &&                        \
    SIW917_ELRS_LR2021_POLL_IRQ_WHEN_NO_DIO
  static uint32_t lr2021PolledIrqCount = 0;
...
#endif
      ;
  const uint32_t lr2021NowUs = micros();
  if (connectionState != connected &&
      (uint32_t)(lr2021NowUs - lr2021LastPollUs) >=
          SIW917_ELRS_LR2021_POLL_IRQ_US) {
    lr2021LastPollUs = lr2021NowUs;
    const uint32_t peekIrq = Radio.PeekIrqStatus(SX12XX_Radio_1);
    if (peekIrq != 0) {
      lr2021PolledIrqCount++;
      lr2021PolledLastIrq = peekIrq;
    }
    if ((peekIrq & lr2021TerminalIrqMask) != 0) {
      ...
      LR1121Driver::IsrCallbackWithStatus(SX12XX_Radio_1, polledIrq);
    }
  }
#if !(ELRS_DIAG_PERIODIC_STATS || ELRS_DIAG_PRINT_AFTER_LOSS)
  (void)lr2021PolledIrqCount;
  (void)lr2021PolledRxCount;
  (void)lr2021PolledLastIrq;
#endif
#endif
  ...
  if (shouldPrintDiag) {
    ...
    uint16_t radioErrors = 0;
    uint8_t radioPacketType = LR20XX_PACKET_TYPE_NONE;
    uint16_t loraRxTotal = 0;
    uint16_t loraCrcError = 0;
    uint16_t loraHeaderCrcError = 0;
    uint16_t loraFalseSync = 0;
    ...
    if (isrCount == 0 && connectionState == disconnected) {
      ...
      radioErrors = Radio.GetErrors(SX12XX_Radio_1);
      radioPacketType = Radio.GetPacketType(SX12XX_Radio_1);
      if (isGfskRate(ExpressLRS_currAirRate_Modparams)) {
        Radio.GetGfskRxStats(SX12XX_Radio_1, &gfskRxTotal, &gfskCrcError,
                             &gfskLenError, &gfskPreamble, &gfskSyncOk,
                             &gfskSyncFail, &gfskTimeout);
    ...
    bool rfScanValid = false;
    int8_t rfScanRssi9150 = 0;
    int8_t rfScanRssi9155 = 0;
    int8_t rfScanRssi9161 = 0;
    static uint32_t lastRfScan = 0;
    if (connectionState == disconnected && isrCount == 0 &&
        now - lastRfScan >= 5000) {
      lastRfScan = now;
      const uint32_t restoreFreq = Radio.currFreq;

      Radio.SetFrequencyReg(915000000UL, SX12XX_Radio_1, true);
      delay(3);
      Radio.StartRssiInst(SX12XX_Radio_1);
      rfScanRssi9150 = Radio.GetRssiInst(SX12XX_Radio_1);

      Radio.SetFrequencyReg(915500000UL, SX12XX_Radio_1, true);
      delay(3);
      Radio.StartRssiInst(SX12XX_Radio_1);
      rfScanRssi9155 = Radio.GetRssiInst(SX12XX_Radio_1);

      Radio.SetFrequencyReg(916100000UL, SX12XX_Radio_1, true);
      delay(3);
      Radio.StartRssiInst(SX12XX_Radio_1);
      rfScanRssi9161 = Radio.GetRssiInst(SX12XX_Radio_1);

      Radio.SetFrequencyReg(restoreFreq, SX12XX_Radio_1, true);
      rfScanValid = true;
    }
    ...
    DBGLN("IRQ isr:%lu rx:%lu tx:%lu other:%lu cb:%lu/%lu rxr:%lu/%lu "
          ...
          "poll:%lu/%lu/0x%08lX txwd:%lu/%lu/%lu/%u/%lu/%lu "
          "rxok:%lu crcfail:%lu okT:%lu/%lu/%lu failT:%lu/%lu/%lu "
          "stat:%u/%02X/%02X/%02X err:0x%04X irssi:%d",
          ...
          (unsigned long)dio2Edges, lastIrq,
#if defined(SIW917_ELRS_LR2021_POLL_IRQ_WHEN_NO_DIO) &&                        \
    SIW917_ELRS_LR2021_POLL_IRQ_WHEN_NO_DIO
          (unsigned long)lr2021PolledIrqCount,
          (unsigned long)lr2021PolledRxCount,
          (unsigned long)lr2021PolledLastIrq,
#else
          0UL, 0UL, 0UL,
#endif
#if SIW917_ELRS_LR2021_TXDONE_WATCHDOG
          ...
          (unsigned)lr2021TelemetryTxPending,
          (unsigned long)lr2021TelemetryTxRfCount,
          (unsigned long)lr2021TelemetryTxNoneCount,
#else
          0UL, 0UL, 0UL, 0U, 0UL, 0UL,
#endif
          pkt_capture_count, crcFailCount,
          (unsigned long)crcPassTypeCount[PACKET_TYPE_RCDATA],
          (unsigned long)crcPassTypeCount[PACKET_TYPE_DATA],
          (unsigned long)crcPassTypeCount[PACKET_TYPE_SYNC],
          (unsigned long)crcFailTypeCount[PACKET_TYPE_RCDATA],
          ...
          radioErrors, instantRssi);
    if (rfScanValid) {
      DBGLN("RFSCAN f:915000000/915500000/916100000 rssi:%d/%d/%d restored:%lu",
            rfScanRssi9150, rfScanRssi9155, rfScanRssi9161,
    ...
    if (connectionState == disconnected) {
      DBGLN("RSTAT ptype:0x%02X lora:%u/%u/%u/%u valid:%u "
            ...
            (unsigned)gfskTimeout, gfskStatsValid ? 1U : 0U);
    }
    hwTimer::service();
  ...
  uint16_t rxLockTimeoutMs = ExpressLRS_currAirRate_RFperfParams->RxLockTimeoutMs;
#if defined(SIW917_ELRS_LR2021_GFSK_RX_LOCK_TIMEOUT_MS) &&                    \
    SIW917_ELRS_LR2021_GFSK_RX_LOCK_TIMEOUT_MS > 0
  if (isGfskRate(ExpressLRS_currAirRate_Modparams) &&
      rxLockTimeoutMs < SIW917_ELRS_LR2021_GFSK_RX_LOCK_TIMEOUT_MS) {
    rxLockTimeoutMs = SIW917_ELRS_LR2021_GFSK_RX_LOCK_TIMEOUT_MS;
  }
#endif

  if ((connectionState == tentative) &&
      ((now - LastSyncPacket) > rxLockTimeoutMs)) {
  ...
  if (connectionState == tentative) {
    const uint8_t lqRaw = LQCalc.getLQRaw();
    const uint8_t minLq = minLqForChaos();
    bool lqEnough = lqRaw > minLq;
#if defined(SIW917_ELRS_LR2021_GFSK_RELAX_TENTATIVE_LQ) &&                    \
    SIW917_ELRS_LR2021_GFSK_RELAX_TENTATIVE_LQ
    if (isGfskRate(ExpressLRS_currAirRate_Modparams) && lqRaw >= minLq) {
      lqEnough = true;
    }
#endif
  ...
  //=========================================================================
  // HP GPIO Radio IRQ Interrupt Mode
  //
  // Core2021-XF DIO11 is connected to GPIO_46 (HP domain) on the SiW917. The
  // copied ELRS path still uses DIO1 naming for its generic radio IRQ callback.
  //
  // The interrupt callback chain is:
  //   DIO11 rising edge -> NVIC IRQ -> dio1_gpio_interrupt_callback()
...
int elrs_format_health_json(char *out, size_t out_len) {
  ...
  return snprintf(
      ...
      (unsigned)LQCalc.getCount(), (int)Radio.LastPacketRSSI,
...
void elrs_rx_test(void) {
  ...
  DBGLN("Radio: LR2021 (Waveshare Core2021-XF)");
