...
bool ICACHE_RAM_ATTR enumRatetoIndex(expresslrs_RFrates_e const eRate, uint8_t &idx)
{
    ...
    {
        expresslrs_mod_settings_s const *const ModParams = get_elrs_airRateConfig(i);
...
uint8_t ICACHE_RAM_ATTR enumRatetoIndexSafe(expresslrs_RFrates_e const eRate)
{
    uint8_t idx;
...
{
    return enumRatetoIndexSafe(eRate);
}
