...
static bool modelIdSavePending = false;
static uint32_t modelIdSaveAtMs = 0;
...
static void DataUlReceiveComplete() {
  ...
  case MSP_ELRS_RXTX_CONFIG:
    if (DataUlBuffer[1] ==
      ...
      const uint8_t requestedModelId =
          DataUlBuffer[2] <= MODELMATCH_MASK ? DataUlBuffer[2] : 0xFF;
      elrs_config_t *cfg = elrs_config_get();
      if (cfg != nullptr && cfg->model_id != requestedModelId) {
        cfg->model_id = requestedModelId;
        modelIdSavePending = true;
        modelIdSaveAtMs = millis() + 500U;
        DBGLN("[RX_CFG] queued model id update=%u", requestedModelId);
      }
      siw917_rx_set_model_match_id(requestedModelId);
...
void elrs_loop(void) {
  ...
  if (modelIdSavePending &&
      (uint32_t)(now - modelIdSaveAtMs) < 0x80000000UL) {
    const int saveResult = elrs_config_save_with_rf_rearm();
    if (saveResult == 1) {
      modelIdSaveAtMs = now + 1000U;
    } else {
      modelIdSavePending = false;
      DBGLN("[RX_CFG] model id save result=%d", saveResult);
    }
  }
  ...
  //
  // DIO1 is wired to the configured HP GPIO on the DK2605A breakout. The interrupt
